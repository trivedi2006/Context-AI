...
def create_db_engine():
    ...
    if not raw_url or "postgresql" not in raw_url or "sqlite" in raw_url:
        logger.critical("CRITICAL ERROR: DATABASE_URL environment variable is missing or invalid! Neon PostgreSQL is required.")
        raise RuntimeError("CRITICAL: DATABASE_URL environment variable is missing or invalid! Neon PostgreSQL connection is required.")

    # Mask credentials for startup banner print
    masked_url = raw_url
    if "@" in raw_url:
    ...
    try:
        ...
        print(f"Database Dialect: {eng.dialect.name}")
        print(f"Server Version: {server_version[:60]}...")
        print("==================================================\n")

        if eng.dialect.name != "postgresql":
            raise RuntimeError(f"CRITICAL: Active database dialect is '{eng.dialect.name}'. PostgreSQL is strictly required!")
